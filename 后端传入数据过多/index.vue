<template>
  <div class="v-scroll" @scroll.passive="doScroll" ref="scrollBox">
    <div :style="blankStyle" style="height: 100%">
      <div v-for="item in tempList" :key="item.id" class="scroll-item">
        <span>{{ item.msg }}</span>
        <img :src="item.src" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    doScroll: throttle(function () {
      ...
      if (this.startIndex + this.itemNum > this.allData.length - 1) {
        this.getAllData(30);
      }
    }, 200),
    getScrollBoxHeight() {
  ...
  computed: {
    ...
    tempList() {
      ...
      return this.allData.slice(startIndex, this.endIndex + 1);
    },
    blankStyle() {
      // 上下方的空白处使用padding来充当
      let startIndex = 0;
      ...
      return {
        ...
        paddingBottom:
          (this.allData.length - this.endIndex - 1) * this.itemHeight + "px",
        // 不要忘了加px哦
      };
    },
  },
